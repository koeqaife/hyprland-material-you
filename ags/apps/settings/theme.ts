...
const ReloadTheme = () => {
    ...
    if (_color.length > 0 &&
        isNumeric(_color)
        && (Number(_color) >= 0 && Number(_color) <= 360)
    ) {
    ...
    if (_color != "none" && _color.length > 6)
        Utils.execAsync(`python -O ${color_generator} --color "${_color}" --color-scheme "${theme.value}" --scheme "${scheme.value}"`)
            .finally(() => { theme_reload_lock = false })
            .then(() => {
                Utils.writeFile(color_to_write, `${GLib.get_home_dir()}/dotfiles/.settings/custom-color`)
                    .catch(print)
            })
            ...
            Widget.Entry({
                ...
                on_change: self => {
                    if (self.text!.length > 6)
                        ...
                        isNumeric(self.text!)
                        && (Number(self.text!) >= 0 && Number(self.text!) <= 360)
                    )
